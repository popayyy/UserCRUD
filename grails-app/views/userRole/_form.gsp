...
<div class="fieldcontain ${hasErrors(bean: userRoleInstance, field: 'role', 'error')} required">
	...
	<g:select id="role" name="role.id" from="${crud.Role.list()}" optionKey="id" required="" value="${userRoleInstance?.role?.id}" class="many-to-one"/>
...
<div class="fieldcontain ${hasErrors(bean: userRoleInstance, field: 'user', 'error')} required">
	...
	<g:select id="user" name="user.id" from="${crud.User.list()}" optionKey="id" required="" value="${userRoleInstance?.user?.id}" class="many-to-one"/>
